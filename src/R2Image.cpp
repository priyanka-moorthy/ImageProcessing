...
ChangeContrast(double factor)
{
  ...
  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
  //calculate average luminanace
  double avgLuminance = 0;
  double sumLuminance = 0;
  for(int i =0; i< npixels;i++){
      sumLuminance = sumLuminance + pixels[i].Luminance();
  }
  avgLuminance = sumLuminance / npixels ;

  //constant gray image
  R2Pixel grayPixel( avgLuminance, avgLuminance, avgLuminance, 1.0);

  for(int i=0; i<npixels; i++){
    pixels[i] = (1 - factor)*grayPixel + factor*pixels[i];
    pixels[i].Clamp();
  }
...
Blur(double sigma)
{
  ...
  //initialize Gaussian Kernel
	...
	std::vector<std::vector<double> > gKernel(size+1, col);

  //populate gKernel
	for(int i=0; i<=size; i++) {
		for(int j=0; j<=size; j++) {
			gKernel[i][j] = exp(-(i*i + j*j) / (2.0 * sigma * sigma));
		}
	}

	for (int i = 0; i < npixels; i++) {
		R2Pixel gaussianPixel(0.0, 0.0, 0.0, pixels[i].Alpha());
		int x0 = i/height;
		int y0 = i%height;
		double total=0;
  //apply Gaussian filter 
		for(int x = (x0 - size < 0 ? 0 : x0 - size); x < (x0 + size + 1 > width ? width : x0 + size + 1); x++) {
			for(int y = (y0 - size < 0 ? 0 : y0 - size); y < (y0 + size + 1 > height ? height : y0 + size + 1); y++) {
				double gKernalVal = gKernel[abs(x-x0)][abs(y-y0)];
				gaussianPixel += gKernalVal*original.Pixel(x,y);
				total += gKernalVal;
			}
		}
		gaussianPixel /= total;
		pixels[i] = gaussianPixel;
	}
    ...
    // apply edge detection mask
		for(int x = x0 - 1; x <= x0 + 1; x++) {
			...
			for(int y = y0 - 1; y <= y0 + 1; y++) {
				...
				if(x == x0 && y == y0) {
					pixel = pixel + (8*originalImage.Pixel(x,y));
					counter += 8;
				} else {
					pixel = pixel + (-1.0*originalImage.Pixel(x,y));
					counter++;
				}
			}
		}
...
R2Pixel R2Image::Sampling(double x0, double y0, int sampling_method, double sigma_x, double sigma_y)
{
	if(sampling_method == R2_IMAGE_POINT_SAMPLING) {
		...
		for(int x = (x_orig - size_x < 0 ? 0 : x_orig - size_x); x < (x_orig + size_x + 1 > width ? width : x_orig + size_x + 1); x++) {
			for(int y = (y_orig - size_y < 0 ? 0 : y_orig - size_y); y < (y_orig + size_y + 1 > height ? height : y_orig + size_y + 1); y++) {
				double gaussianXY = exp(-(x-x0)*(x-x0) / 2.0 / sigma_x / sigma_x) * exp(-(y-y0)*(y-y0) / 2.0 / sigma_y / sigma_y);
				pixel += gaussianXY*Pixel(x,y);
				total += gaussianXY;
			}
		}
		pixel /= total;
...
R2Image originalImage(*this);
	width = lround(sx*originalImage.width);
	height = lround(sy*originalImage.height);

	delete [] pixels;
	npixels = width*height;
	pixels = new R2Pixel[npixels];

	double xoffset = 0.5 * ((double)originalImage.width) / ((double)width) - 0.5;
	double yoffset = 0.5 * ((double)originalImage.height) / ((double)height) - 0.5;
	
	for(int i=0; i<npixels; i++) {
		int x0 = i/height;
		int y0 = i%height;

		double x_orig = ((double)originalImage.width) / ((double)width) * x0 + xoffset; 
		double y_orig = ((double)originalImage.height) / ((double)height) * y0 + yoffset; 

		double sigma_x = 1.0/3.0/sx, sigma_y = 1.0/3.0/sy;
		if(sx > 1.0) { sigma_x = 0.5; }
		if(sy > 1.0) { sigma_y = 0.5; }
    //sampling
		pixels[i] = originalImage.Sampling(x_orig, y_orig, sampling_method, sigma_x, sigma_y);
	}
  // FILL IN IMPLEMENTATION HERE (REMOVE PRINT STATEMENT WHEN DONE)
